...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    const ent = Object.entries(objects[i]);
    for (let j = 0; j < ent.length; j += 1) {
      if (entries.has(ent[j][0])) {
        const sum = entries.get(ent[j][0]) + ent[j][1];
        entries.set(ent[j][0], sum);
      } else {
        entries.set(ent[j][0], ent[j][1]);
      }
    }
  }
...
function sortCitiesArray(arr) {
  return arr.sort((geoA, geoB) => {
    const result = geoA.country
      .toLowerCase()
      .localeCompare(geoB.country.toLowerCase());
    if (result !== 0) {
      return result;
    }
    return geoA.city.toLowerCase().localeCompare(geoB.city.toLowerCase());
  });
}
